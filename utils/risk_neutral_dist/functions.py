...
def get_pdf(cdf_pricer, ttms, strikes, is_calls, EPS=1e-10):
    ...
    returns: 2d array of floats, each row ttm provding the pdf value at each strike
    """
    return np.array([[
        (
...
def get_discrete_pdf(cdf_pricer, ttms, strikes, is_calls):
    """
    ttms: array of floats of times to maturity
    strikes: array of floats of strikes
    
    returns: 2d array of floats, each row ttm provding the pdf value between strikes
    - this is the cumulative probability density over the range between each strike
    """
    strike_buckets = list(zip(strikes, strikes[1:]))
    pdf = np.array([[
        (
            cdf_pricer.risk_neutral_cdf(T=ttm, K=sb[1], is_call=is_calls)
            - cdf_pricer.risk_neutral_cdf(T=ttm, K=sb[0], is_call=is_calls)
        )
        for sb in strike_buckets
    ] for ttm in ttms])
    strikes = np.array([np.mean(sb) for sb in strike_buckets]).reshape(1, -1)
    return pdf, strikes
